...
class GBNHost():
    ...
    def receive_from_network_layer(self, byte_data):
        if self.is_ack(byte_data) and not self.is_corrupt(byte_data):
            ackNUM = self.get_ack_num(byte_data)
            if ackNUM >= self.window_base:
                ...
                self.simulator.pass_to_network_layer(self.entity, self.last_ACK_pkt, self.is_ack(self.last_ACK_pkt))
        elif self.get_seq_num(byte_data) != self.expected_seq_number:
                self.simulator.pass_to_network_layer(self.entity, self.last_ACK_pkt, self.is_ack(self.last_ACK_pkt))
        else:
            try:
            ...
            self.last_ACK_pkt = self.make_pkt(self.expected_seq_number, "ACK")
            self.simulator.pass_to_network_layer(self.entity, self.last_ACK_pkt, self.is_ack(self.last_ACK_pkt))
    ...
    def is_corrupt(self, packet):
        ...
        if checksum == self.make_cs(pkt):
            return False
        else: 
            return True

    def make_cs(self, pkt):
        if len(pkt) % 2 != 0:
            pkt = pkt + bytes(1)
        sum_words = 0
    ...
    def get_ack_num(self, pkt):
        header = unpack("!HiHI" , pkt[:12])
        return header[1]

    def get_seq_num(self,pkt):
        header = unpack("!HiHI" , pkt[:12])
        return header[1]
